...
> **Goal:** Tích hợp khả năng multi-agent team orchestration, dynamic workflow, và tmux/dmux-based parallel execution vào deepcode-cli. Biến kiến trúc single-agent hiện tại thành nền tảng Agent lập trình đa tác tử có khả năng song song hóa, mà không phá vỡ bất kỳ chức năng hiện có nào.
...
6. **Session persistence:** JSONL file per session (`~/.deepcode/projects/<projectCode>/<sessionId>.jsonl`)
...
```bash
deepcode --team -p "refactor entire codebase to TypeScript strict mode"
```
...
## Mode 2: Tmux Visual
```bash
deepcode --team --tmux -p "build auth + api + frontend simultaneously"
```
- Mỗi worker trong 1 tmux pane
- Coordinator ở pane trên cùng
- User có thể attach: `tmux attach -t deepcode-team-{uuid}`
- Layout:
```
┌──────────────────────────────────────┐
│         Coordinator (pane 0)         │
...
```bash
deepcode --team --dmux -p "run full CI pipeline"
```
...
```bash
deepcode --team --yolo --output team-result.json -p "process task queue"
```
- Không interactive
- Output JSON ra file
...
| **M7: CLI + Skills** | Modify `cli.tsx`, `settings.ts`, `permissions.ts`, `PromptInput.tsx`; 2 skill files | `deepcode --team -p "..."` hoạt động |
