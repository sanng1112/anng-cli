# Add Gemini Compatibility to README Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Status:** Completed

**Goal:** Add a detailed section to `README.md` documenting the project's native compatibility with Google Gemini models, explaining how the conversion of tool messages to the user role and the bypass of the thinking parameter are handled under the hood.

**Architecture:** We will modify `README.md` to introduce a new section: "Hỗ trợ và tương thích với Google Gemini". We will document the configuration parameters (`GEMINI_API_KEY`, `GEMINI_BASE_URL`) and the built-in translation details.

**Tech Stack:** Markdown, Git.

---

### Task 1: Update README.md with Gemini Compatibility Section

**Files:**
- Modify: `README.md`

- [x] **Step 1: Update README.md content**

Replace the contents of `README.md` to insert the Gemini section:
...
<h1>ANNG CLI CLI (Enhanced Agentic Version)</h1>
...
**ANNG CLI CLI (Enhanced Agentic Version)** là phiên bản nâng cấp mạnh mẽ từ trợ lý lập trình AI chạy trên terminal tối ưu cho `deepseek-v4`. So với phiên bản gốc, phiên bản này được thiết kế và tối ưu hóa vượt trội để đóng vai trò như một **Agent lập trình tự trị (Autonomous Developer Agent)** với hệ thống harness kiểm soát thực thi, phục hồi lỗi, quản lý ngữ cảnh thông minh và công cụ xử lý file hiệu năng cao.
...
*   **Hàng đợi tác vụ tự trị (Headless Task Queue):** Hỗ trợ chạy Agent ở chế độ không giao diện (headless) bằng cách tự động tải và xử lý danh sách hàng đợi tác vụ được lưu trữ dưới dạng file tại đường dẫn `.anng/memory/task-queue.md` khi chế độ `autoAccept` được kích hoạt.
...
## ♊ Hỗ Trợ Và Tương Thích Với Google Gemini

ANNG CLI CLI hỗ trợ chạy trực tiếp với các mô hình của dòng **Google Gemini** (ví dụ: `gemini-3.5-flash`, `gemini-3.1-flash-lite`, `gemini-2.5-flash`, `gemini-2.5-flash-lite`) thông qua API tương thích với cấu trúc của OpenAI. 

Để giải quyết các xung đột định dạng giao tiếp giữa OpenAI và Gemini, dự án đã triển khai tích hợp sẵn các cơ chế xử lý tương thích tự động:

*   **Chuyển đổi vai trò Tool sang User (Tool-to-User Role Mapping):** Endpoint tương thích OpenAI của Gemini không hỗ trợ vai trò tin nhắn là `tool`. CLI sẽ tự động ánh xạ và dịch chuyển các tin nhắn trả về của công cụ sang vai trò `user` trước khi gửi lên API để ngăn chặn lỗi định dạng 400 Bad Request.
*   **Loại bỏ tham số Tư duy sâu (Thinking Parameter Bypass):** Do Gemini không hỗ trợ tham số cấu hình suy luận (`thinking`) của DeepSeek, CLI tự động nhận dạng tiền tố mô hình `gemini` và loại bỏ tham số này khi khởi tạo yêu cầu.
*   **Cấu hình API Key và Endpoint độc lập:** Bạn có thể thiết lập cấu hình riêng cho Gemini trong file `settings.json` hoặc sử dụng các biến môi trường:
    *   `GEMINI_API_KEY`: API Key kết nối với Google AI Studio.
    *   `GEMINI_BASE_URL`: Mặc định trỏ đến `https://generativelanguage.googleapis.com/v1beta/openai/`.
...
Tạo liên kết hệ thống để bạn có thể chạy lệnh `anng` trực tiếp ở bất kỳ thư mục nào:
```bash
npm link
```
...
Tạo file cấu hình tại đường dẫn `~/.anng/settings.json` với nội dung như sau:
...
Nếu bạn muốn chuyển sang sử dụng mô hình Gemini:
...
> **Lưu ý:** File cấu hình này được chia sẻ chung với [ANNG CLI VSCode Extension](https://github.com/lessweb/anng).
...
### ANNG CLI có Plugin dành cho VSCode không?
Có. ANNG CLI cung cấp một extension đầy đủ tính năng cho VSCode. Bạn có thể cài đặt tại [VSCode Marketplace](https://marketplace.visualstudio.com/items?itemName=vegamo.anng-vscode). Extension này dùng chung cấu hình `~/.anng/settings.json` với CLI giúp bạn chuyển đổi mượt mà giữa terminal và editor.
...
ANNG CLI CLI tích hợp sẵn một công cụ tìm kiếm web miễn phí và đủ dùng cho hầu hết trường hợp. Nếu bạn muốn sử dụng script tìm kiếm tùy chỉnh của riêng mình, hãy thiết lập trường `webSearchTool` trong `settings.json` chỉ tới đường dẫn script của bạn. Xem chi tiết tại: https://github.com/qorzj/web_search_cli

### Cấu hình máy chủ MCP (Model Context Protocol) như thế nào?
ANNG CLI hỗ trợ giao thức MCP để kết nối trợ lý AI với GitHub, trình duyệt web, cơ sở dữ liệu hoặc các dịch vụ bên ngoài khác. Chỉ cần thêm cấu hình trong trường `mcpServers` của file `settings.json`. Sau đó, dùng lệnh `/mcp` trong CLI để kiểm tra danh sách các công cụ đang hoạt động. Tham khảo chi tiết tại [docs/mcp.md](docs/mcp.md).

### Phân quyền hoạt động của AI như thế nào để đảm bảo an toàn?
ANNG CLI không bắt buộc chạy ở chế độ YOLO (tự động chạy lệnh nguy hiểm). Dự án tích hợp hệ thống phân quyền tinh vi. Trước khi AI thực thi shell, đọc/ghi tệp tin ngoài thư mục dự án hoặc truy cập mạng, hệ thống sẽ hiển thị hộp thoại xác nhận. Bạn có thể tùy chỉnh các chính sách phân quyền này (`allow`, `deny`, `ask`) trong mục `permissions` của file `settings.json`. Xem chi tiết tại [docs/permission.md](docs/permission.md).
...
```bash
git add README.md
git commit -m "docs: document Gemini compatibility features in README.md"
```
