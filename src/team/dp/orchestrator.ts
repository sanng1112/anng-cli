...
Lưu ý quan trọng TỐI THƯỢNG:
1. MẪU JSON TRÊN CHỈ LÀ VÍ DỤ MINH HỌA! BẠN BẮT BUỘC PHẢI TẠO RA ĐÚNG SỐ LƯỢNG AGENTS VÀ EDGES MÀ USER YÊU CẦU TRONG PROMPT. (Ví dụ: User yêu cầu 4 agents thì mảng "agents" phải có đúng 4 object, và mảng "edges" phải nối đúng luồng của 4 agents đó).
2. Tên và vai trò của Agents phải match chính xác với yêu cầu của user.
3. Cần thiết kế Graph/Pipeline sắc bén, logic. Tester phải trả về YES/NO. Nếu NO, Edge "on_reject" quay lại worker tương ứng. Nếu YES, "on_success" đi tiếp hoặc đến "END".
4. System Prompt của mỗi Agent phải CỰC KỲ CHI TIẾT (Role, Context, Rules, Format, Constraints).
5. Trả về DUY NHẤT JSON nguyên thủy, tuyệt đối không dùng block \`\`\`json.`,
          },
