...
test("getSystemPrompt does not include current date guidance", () => {
  const now = new Date();
  const expected = `Today is ${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}. Time passes as the conversation continues.`;
  const prompt = getSystemPrompt("/tmp/project");
...
test("getRuntimeContext includes current date and model guidance", () => {
  const now = new Date();
  const expectedDate = `Today is ${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}. Time passes as the conversation continues.`;
  const prompt = getRuntimeContext("/tmp/project", "deepseek-v4-pro");
  assert.equal(prompt.includes(expectedDate), true);
  assert.equal(
    prompt.includes("The current LLM model is deepseek-v4-pro. Switch models with the /model command."),
    true
  );
