...
test("restoreSessionConversation truncates messages before the selected user prompt", async () => {
  ...
  const sessionId = await manager.createSession({ text: "first prompt" });
  const firstAssistant = buildAssistantMessage(sessionId, "first answer", null, null) as SessionMessage;
  ...
  const secondAssistant = buildAssistantMessage(sessionId, "second answer", null, null) as SessionMessage;
...
test("restoreSessionCode preserves files that predate their first tracked mutation", async (t) => {
  ...
  fs.writeFileSync(readmePath, "This is a hello world demo project\n", "utf8");
  ...
  assert.equal(fs.readFileSync(readmePath, "utf8"), "This is a hello world demo project\n");
...
test("replySession /continue runs trailing pending tool calls before requesting another response", async () => {
  ...
  const sessionId = await manager.createSession({ text: "first prompt" });
  const pendingAssistant = buildAssistantMessage(
    sessionId,
    "Need to read a file",
    [
      {
...
test("buildOpenAIMessages keeps only the first non-interrupted tool result for a tool call", () => {
  ...
  const successToolMessage = (manager as any).buildToolMessage(
    "session-1",
    "call-1",
    JSON.stringify({ ok: true, name: "bash", output: "2026-05-07 Thursday\n" }),
...
test("SessionManager active tokens reflect latest response usage with tokenizer-based compaction", async () => {
  ...
  const sessionId = await manager.createSession({ text: "test ".repeat(50000) });
  assert.equal(manager.getSession(sessionId)?.activeTokens, 140_000);

  await manager.replySession(sessionId, { text: "" });

  const session = manager.getSession(sessionId);
  // With tokenizer-based compaction, large user message triggers compaction
...
test("SessionManager streams chat completions and counts reasoning progress", async () => {
  ...
  assert.equal((assistantMessage?.messageParams as any)?.reasoning_content, "thinking");
