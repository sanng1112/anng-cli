...
import { shouldCompactContext, truncateOldToolMessages } from "./context-compacter";
...
import {
  buildUserMessage as buildUserMsgExternal,
  buildToolMessage as buildToolMsgExternal,
  normalizeSessionMessage as normalizeSessionMsgExternal,
  generateToolCallId as generateToolCallIdExternal,
  normalizeLlmToolCalls as normalizeLlmToolCallsExternal,
} from "./message-manager";

import { globalFileWriteQueue } from "./file-write-queue";
...
`;
  ...
  private truncateOldToolMessages(sessionId: string): void {
    ...
    truncateOldToolMessages(messages, (updatedMessages) => {
  ...
  private normalizeSessionMessage(message: SessionMessage): SessionMessage {
    return normalizeSessionMsgExternal(message, this.projectRoot);
  }
  ...
  private buildUserMessage(sessionId: string, prompt: UserPromptContent): SessionMessage {
    return buildUserMsgExternal(sessionId, prompt, (sid) => this.getCurrentCheckpointHash(sid));
  }
  ...
  private generateToolCallId(): string {
    return generateToolCallIdExternal();
  }

  private normalizeLlmToolCalls(rawToolCalls: unknown[] | null | undefined): unknown[] | null {
    return normalizeLlmToolCallsExternal(rawToolCalls, () => this.generateToolCallId());
  }
  ...
  private buildToolMessage(
    ...
    return buildToolMsgExternal(sessionId, toolCallId, content, toolFunction, this.projectRoot);
  }
