...
type QueueViewProps = {
  projectRoot: string;
  onExit: () => void;
  onProcessTask: (taskText: string, taskId: string, queueName: string) => void;
  screenWidth: number;
...
export const QueueView = React.memo(function QueueView({
  ...
  const refreshQueues = useCallback(() => {
    setQueues(listQueues(projectRoot));
  }, [projectRoot]);
  ...
  const filteredHistory = useMemo(
    () => promptHistory.filter((h) => h.length > 0 && !h.startsWith("/")),
    [promptHistory]
  );
    ...
    (input, key) => {
      ...
      if (screen === "edit") {
        ...
        if (key.return) {
          ...
          if (trimmed && activeQueue && editIndex >= 0) {
            ...
            showStatus(`Updated: "${trimmed.slice(0, 60)}"`);
            refreshTasks();
          }
          setInputBuffer("");
          setScreen("list");
      ...
      if (screen === "confirm-clear") {
        ...
        if (key.return && activeQueue) {
          clearQueue(projectRoot, activeQueue.name);
      ...
      if (showHistory) {
        ...
        if (key.return && filteredHistory.length > 0 && activeQueue) {
          ...
          if (sel) {
            ...
            showStatus(`Pulled: "${sel.slice(0, 60)}"`);
            refreshTasks();
            refreshQueues();
          }
          setShowHistory(false);
        ...
        if (key.downArrow) {
          setHistoryIndex((i) => Math.min(filteredHistory.length - 1, i + 1));
          return;
        }
        return;
      }
      ...
      if (key.tab && queues.length > 0) {
        switchQueue((activeQueueIdx + 1) % queues.length);
        return;
      }
      if (/^[1-9]$/.test(input)) {
      ...
      if (input === "p" || input === "P") {
        if (activeQueue) {
          ...
          if (next) onProcessTask(next.text, next.id, activeQueue.name);
          else showStatus("No pending tasks");
        }
        return;
      }
      if (input === "q" || input === "Q") {
        setScreen("queue-menu");
        setInputBuffer("");
        return;
      }
      if (input === "v" || input === "V") {
