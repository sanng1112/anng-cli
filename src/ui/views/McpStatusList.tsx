...
export function McpStatusList({ statuses, onCancel, onReconnect }: Props): React.ReactElement {
  ...
  // Enter server detail (allow ready, failed, reconnecting states)
  ...
  // When no servers, listen for Esc to exit
  useInput((input, key) => {
...
function ServerListView({
  ...
  const maxVisible = useMemo(() => {
    ...
    // Each server takes 1 row (title) + 1 row (error/stat) + 1 row (spacing)
    return Math.max(1, Math.floor(availableLines / 3));
  }, [rows]);

  // Calculate label column width: find longest server name, add prefix and icon
  const labelColumnWidth = useMemo(() => {
    ...
    const maxAllowed = Math.max(15, Math.floor((columns - 6) * 0.4)); // Container 40% width, at least 15 columns
  ...
  // Auto-scroll to ensure selected item is visible
  React.useEffect(() => {
...
function ServerRow({
  ...
  // Loading animation: cycle (empty) -> . -> .. -> ... -> (empty) -> ...
...
function ErrorRow({ error }: { error: string }): React.ReactElement {
  // Split error message by lines, display each line separately
