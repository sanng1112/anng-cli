...
export const PromptInput = React.memo(function PromptInput({
  ...
  const busyStatusText =
    loadingText && loadingText.trim()
      ...
      : `esc to interrupt · ctrl+c to cancel input/exit${processOrPasteHint}`;
  ...
  return (
    <Box flexDirection="column" width={screenWidth}>
      ...
      <Box flexDirection="row" justifyContent="space-between" width={inputContentWidth}>
        <Box>
          {executionMode === "autoAccept" ? (
            <Text color="red">Auto Mode </Text>
          ) : executionMode === "plan" ? (
            <Text color="blue">Plan Mode </Text>
          ) : (
